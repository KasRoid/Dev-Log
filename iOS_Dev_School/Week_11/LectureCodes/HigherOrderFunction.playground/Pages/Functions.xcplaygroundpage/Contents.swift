...
/*:
 ...
 ---
 */
// forEach 는 함수이므로 return 과 같은 제어문을 사용할 수 있으며 반복문에 사용하는 break, continue 등을 사용할 수 없다.

let oneToTenArray = (1...10)
oneToTenArray.forEach() {
    guard $0 % 2 == 0 && $0 <= 9 else { return }
    print($0)
}
...
let oneToNineArray = (1...9)
let doubledOneToNineArray = oneToNineArray.map() { $0 * 2 }
print(doubledOneToNineArray)
/*:
 ---
 ## filter
...
let myArray = Array(1...50).filter { $0 % 3 == 0 }
print(myArray)
...
print(["1", "2", "3"].reduce("") { $0 + $1 })



print([1, 2, 3].reduce("") { $0 + String($1) })
